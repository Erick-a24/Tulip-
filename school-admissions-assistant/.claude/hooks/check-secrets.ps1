...
# Ported from the personal bot's earlier "Barry" incarnation — same rule, same logic,
# now protecting The Admin.
param()
...
foreach ($p in $patterns) {
    if ($text -match $p) {
        Write-Error "ADMIN SAFETY: Looks like a secret or key is about to be written to a file. Remove it and use a placeholder like '<your-key-here>' instead."
        exit 2
    }
}
