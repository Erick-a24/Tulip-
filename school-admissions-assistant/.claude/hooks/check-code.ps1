...
# Red    (exit 2) - broken, undo and mark the job stuck
# Ported from Barry's check-code.ps1, unchanged logic.
param()
...
if ($ext -eq '.ps1') {
    ...
    if ($errors -and $errors.Count -gt 0) {
        ...
        Write-Host "   This change should be undone and the job marked stuck."
        exit 2
    }
